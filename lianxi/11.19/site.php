...
class Site extends IController
{
	...
	public function reg_ok(){
		...
		$user1->setData($data);
		if($id=$user1->add()){
			$id_new=urlencode(base64_encode($id));
			$mail=new SendMail();
			$title='账号激活';
			$time=time();
	...
	public function login_ok(){
		...
		$sql = "select * from user2 where name='$name' && password='$password' && status=1";
		$user2=new IModel('user2');
		if($user2->find("name='$name' && password='$password' && status=1")){
			echo "登陆成功";
			return false;
		}else{
			echo "登陆失败";
